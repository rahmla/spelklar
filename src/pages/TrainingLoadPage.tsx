...
export function TrainingLoadPage({ profile, onDone }: Props) {
  ...
  const load = rpe > 0 && duration > 0 ? rpe * duration : null
  ...
  if (done) {
    return (
      <div className="min-h-screen bg-gray-950 flex flex-col items-center justify-center gap-3">
        ...
        {load && <p className="text-gray-400 text-sm">Belastning: {load} poäng</p>}
      </div>
    )
  }
      ...
      <div className="flex flex-col gap-4 px-4">
              ...
              return (
                <button
                  ...
                  className={[
                    ...
                    rpe === n ? `${color} text-white` : 'bg-gray-800 text-gray-400 hover:bg-gray-700',
                  ].join(' ')}
                >
                  {n}
                </button>
        ...
        {load !== null && (
          <div className="bg-orange-900/20 border border-orange-700/40 rounded-2xl p-4">
            ...
            <p className="text-white font-bold text-2xl">{load} <span className="text-gray-500 font-normal text-sm">poäng</span></p>
            <p className="text-gray-500 text-xs mt-1">{duration} min × RPE {rpe} = {load}</p>
          </div>
        )}
